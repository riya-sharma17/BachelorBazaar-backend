...
const generateAndSendOTP = async (user: any): Promise<boolean> => {
    try {
        const otp = generateOTP();
        const otpExpires = moment().add(2, "minutes").toDate();
...
export const googleLogin = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!user) {
            ...
            const alreadyLinked = user.socialIds?.some(
                (s: any) => s.id === googleId && s.type === loginType.GOOGLE
            );
...
export const resetPassword = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const user = await userModel.findOne({ email });
        if (!user || user.OTP !== otp || !user.otpExpires || moment().isAfter(user.otpExpires)) {
            return res.status(400).json({
                message: ERROR_RESPONSE.INVALID_OR_EXPIRED_OTP,
            });
        }
